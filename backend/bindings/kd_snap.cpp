...
// kd_nodes.bin format: [uint32 numNodes] [ float32 lat_0, float32 lon_0, uint32
// idx_0 ] … repeated
void LoadKD(const std::string& path)
{
  ...
  tree =
      bgi::rtree<Value, bgi::quadratic<16>>(allKDData.begin(), allKDData.end());
}
...
{
  std::unordered_set<uint32_t> allowedSet(allowedIndices.begin(),
                                          allowedIndices.end());
  ...
  tree = bgi::rtree<Value, bgi::quadratic<16>>(filteredData.begin(),
                                               filteredData.end());
}
...
Napi::Value FindNearest(const Napi::CallbackInfo& info)
{
  ...
  {
    Napi::TypeError::New(env, "Expected (number, number)")
  ...
  if (result.empty())
  {
    Napi::Error::New(env, "R-tree is empty or no points match filter")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
...
Napi::Value FilterKD(const Napi::CallbackInfo& info)
{
  ...
  {
    Napi::TypeError::New(env, "Expected (array of node indices)")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  for (uint32_t i = 0; i < jsArray.Length(); ++i)
  {
    ...
    if (!val.IsNumber())
    {
      Napi::TypeError::New(env, "Array must contain numbers")
          .ThrowAsJavaScriptException();
      return env.Null();
    }
