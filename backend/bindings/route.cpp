...
struct SurfaceTypeFilter
{
  ...
  SurfaceTypeFilter(types::BitWidth allowedMask, const Graph& graph)
      : allowedSurfaceMask(allowedMask), g(&graph)
  {}

  bool operator()(const Graph::edge_descriptor& e) const
  {
    return (allowedSurfaceMask &
            static_cast<types::BitWidth>((*g)[e].surface)) != 0;
  }
};
...
{
  std::cerr << "at top [C++] Memory used: "
            << getMemoryUsageInBytes() / (1024.0 * 1024.0) << " MB\n";
  std::ifstream nodesIn(nodesPath, std::ios::binary),
      edgesIn(edgesPath, std::ios::binary);
  if (!nodesIn && !edgesIn)
  {
    throw std::runtime_error(
        "Could not open nodes or edges file: " + nodesPath + edgesPath);
  }
  ...
  {
    for (uint32_t edgeIdx = offsets[nodeIdx]; edgeIdx < offsets[nodeIdx + 1];
         ++edgeIdx)
    {
      // boost::add_edge(nodeIdx, neighbors[edgeIdx],
      // EdgeProps{weights[edgeIdx], surfaces[edgeIdx]}, graph);
      auto neighbor = neighbors[edgeIdx];
      if (nodeIdx < neighbor)
      {
        boost::add_edge(nodeIdx, neighbor,
                        EdgeProps{weights[edgeIdx], surfaces[edgeIdx]}, graph);
      }
    }
  }
  std::cerr << "before fGrapoph.emplace[C++] Memory used: "
            << getMemoryUsageInBytes() / (1024.0 * 1024.0) << " MB\n";
  fGraph.emplace(graph, SurfaceTypeFilter(types::ALL_SURFACES, graph));
  std::cerr << "after fGrapoph.emplace[C++] Memory used: "
            << getMemoryUsageInBytes() / (1024.0 * 1024.0) << " MB\n";
  std::cerr << "[route] Loaded graph with " << numNodes << " nodes and "
            << numEdges << " edges\n";
}
...
Napi::Value BuildFilteredGraph(const Napi::CallbackInfo& info)
{
  ...
  {
    Napi::TypeError::New(env, "Expected a single numeric surface type bitmask")
        .ThrowAsJavaScriptException();
    return env.Null();
  }

  types::BitWidth mask =
      static_cast<types::BitWidth>(info[0].As<Napi::Number>().Uint32Value());

  // 3. Validate that the base graph is initialized
  if (boost::num_vertices(graph) == 0)
  {
    Napi::Error::New(env, "Base graph is not initialized")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  std::cerr << "[route] Filtered graph rebuilt with surface mask: 0x"
...
Napi::Value FindPath(const Napi::CallbackInfo& info)
{
  ...
  if (!fGraph.has_value())
  {
    Napi::Error::New(env, "Routing graph not initialized")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  {
    Napi::RangeError::New(env, "startIdx/endIdx out of range [0.." +
                                   std::to_string(numNodes - 1) + "]")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  try
  {
    ...
    for (uint32_t i{0}; i < numNodes; ++i) pred[i] = i;
    // debug version
    ...
    boost::dijkstra_shortest_paths(
        *fGraph, start,
        boost::weight_map(wmap).distance_map(dmap).predecessor_map(pmap));
    // boost::dijkstra_shortest_paths(*fGraph, start,
    // boost::predecessor_map(&pred[0]).distance_map(&dist[0]));
    // boost::dijkstra_shortest_paths(
    //     *fGraph, start,
    //     boost::weight_map(boost::get(&EdgeProps::weight, *fGraph))
    //         .distance_map(boost::make_iterator_property_map(dist.begin(),
    //         boost::get(boost::vertex_index, *fGraph))) .predecessor_map(
    //             boost::make_iterator_property_map(pred.begin(),
    //             boost::get(boost::vertex_index, *fGraph))));
    std::cerr << "[route] dist[end]=" << dist[end]
              << ", pred[end]=" << pred[end] << "\n";
...
Napi::Value GetFullGraphSegments(const Napi::CallbackInfo& info)
{
  ...
  // if (!fGraph.has_value()) {
  //     Napi::Error::New(env, "Filtered graph not
  //     initialized").ThrowAsJavaScriptException(); return env.Null();
  // }
  ...
  std::cerr << "[route] getFullGraphSegments returned " << edgeCount
            << " segments\n";
...
Napi::Value GetFilteredNodeIndices(const Napi::CallbackInfo& info)
{
  ...
  if (!fGraph.has_value())
  {
    Napi::Error::New(env, "Filtered graph not initialized")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
...
Napi::Object Init(Napi::Env env, Napi::Object exports)
{
  ...
  exports.Set("buildFilteredGraph",
              Napi::Function::New(env, BuildFilteredGraph));
  exports.Set("getFilteredNodeIndices",
              Napi::Function::New(env, GetFilteredNodeIndices));
  // debug code
  exports.Set("getFullGraphSegments",
              Napi::Function::New(env, GetFullGraphSegments));
  return exports;
}
