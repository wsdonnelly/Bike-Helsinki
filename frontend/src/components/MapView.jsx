...
import {
  MapContainer, TileLayer, Marker, Polyline, useMapEvents, ZoomControl
} from 'react-leaflet';
...
function splitRuns(coords, modes, modeBit) {
  const out = [];
  if (!coords || coords.length < 2 || !modes || modes.length !== coords.length - 1) {
    return out;
  }
  let run = [];
...
export function MapView({
  ...
  return (
    ...
    >
      ...
      {snappedStart && <Marker position={[snappedStart.lat, snappedStart.lon]} />}
