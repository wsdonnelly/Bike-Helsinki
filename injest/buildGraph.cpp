...
void writeGraphNodesBin(
    uint32_t numNodes, const std::vector<uint64_t>& allNodeIds,
...
struct WayCollector : public osmium::handler::Handler
{
  ...
  // could be constexpr arr? move comments to documentation README
...
struct NodeCollector : public osmium::handler::Handler
{
  ...
  void node(const osmium::Node& node)
  {
    ...
    {
      nodeCoordMap[id] = {static_cast<float>(node.location().lat()),
...
double haversine(double lat1, double lon1, double lat2, double lon2)
{
  ...
  double centralAngle = 2.0 * std::atan2(std::sqrt(haversineValue),
...
int main(int argc, char* argv[])
{
  ...
  // CSR arrays:
  //   vector<uint32_t> offsets(numNodes+1);  // offsets[i] = start index in
  //   "edges" vector<uint32_t> neighbors(numEdges); vector<float>
  //   weights(numEdges);
